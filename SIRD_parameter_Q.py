...
def visualize(true_y, pred_y, param, q, odefunc, itr, t):
    ...
    if args.viz:
        ...
        ax_cita.plot(t.numpy(), torch.full([len(t.numpy())], param[0]).numpy(), 'r--')

        ax_ep.cla()
        ax_ep.set_title('Ep')
        ax_ep.set_xlabel('t')
        ax_ep.set_ylabel('Epp(t)')
        ax_ep.plot(t.numpy(), torch.full([len(t.numpy())], param[1]).numpy(), 'r--')

        fig.tight_layout()
        plt.savefig('png/{:03d}'.format(itr))
        
        np.savetxt('txt/SIRD_NN' + args.areas + str(itr) + '.csv', pred_y.numpy()[:, 0, 0, :], delimiter=',')
...
class ODEFunc(nn.Module):
    ...
    def forward(self, t, y):
        ...
        Y = torch.zeros([1,1,3])
        Y[:,:,0] = beta*(N - y[:,:,0] - y[:,:,1] - y[:,:,2]) * y[:, :, 0]/N - torch.abs(self.cita)*y[:,:,0] - torch.abs(self.ep)*y[:,:,0]
...
if __name__ == '__main__':
    ...
    func = ODEFunc()
    params = (list(func.parameters()) + list([func.cita, func.ep]))
