...
#countries
    ...
    true_y0 = torch.tensor([[0.0002, 0.00, 0.00]]) # I, R, D
    N = 5652.0002
    data = np.loadtxt('South_africa_IRD.csv', delimiter=',', skiprows=1)

data_size = data.shape[0]-args.prob
tp = data[0:data_size, 0] # t
true_yp = data[0:data_size, 1:4] # I, R, D
t = np.linspace(0, len(tp)-1, data_size)
true_y = np.full((data_size, true_yp.shape[1]), 1.0)
...
def visualize(true_y, pred_y, param, odefunc, itr, t):
    ...
    if args.viz:
        ...
        np.savetxt('txt/SIRD_pmter' + args.areas + str(itr) + '.csv', pred_y.numpy()[:, 0, 0, :], delimiter=',')
        print('I-R2:', r2_score(true_y.numpy()[:, 0, 0]*scale, pred_y.numpy()[:, 0, 0, 0]*scale))
        print('R-R2:', r2_score(true_y.numpy()[:, 0, 1]*scale, pred_y.numpy()[:, 0, 0, 1]*scale))
        print('D-R2:', r2_score(true_y.numpy()[:, 0, 2]*scale, pred_y.numpy()[:, 0, 0, 2]*scale))
        print('I-MSE:', mean_squared_error(true_y.numpy()[:, 0, 0]*scale, pred_y.numpy()[:, 0, 0, 0]*scale))
        print('R-MSE:', mean_squared_error(true_y.numpy()[:, 0, 1]*scale, pred_y.numpy()[:, 0, 0, 1]*scale))
        print('D-MSE:', mean_squared_error(true_y.numpy()[:, 0, 0]*scale, pred_y.numpy()[:, 0, 0, 0]*scale))
...
class ODEFunc(nn.Module):
    ...
    def forward(self, t, y):
        ...
        Y = torch.zeros([1,1,3])
        Y[:,:,0] = torch.abs(self.beta)*(N - y[:,:,0] - y[:,:,1] - y[:,:,2]) * y[:, :, 0]/N - torch.abs(self.cita*y[:,:,0]) - torch.abs(self.ep)*y[:,:,0]
        Y[:,:,1] = torch.abs(self.cita)*y[:,:,0]
        Y[:,:,2] = torch.abs(self.ep)*y[:,:,0]
        return Y
...
if __name__ == '__main__':
    ...
    func = ODEFunc()
    optimizer = optim.RMSprop([func.beta, func.cita, func.ep], lr=args.lr)
    end = time.time()
    ...
    for itr in range(1, args.niters + 1):
        ...
        if itr % args.test_freq == 0:
            ...
            with torch.no_grad():
                ...
                loss = loss_I + loss_R + loss_D
                param = [func.beta.item(), func.cita.item(), func.ep.item()]
