...
if args.viz:
    ...
    ax_beta = fig.add_subplot(247, frameon=False)
...
def visualize(true_y, pred_y, param, q, odefunc, itr, t):
    ...
    if args.viz:
        ...
        ax_C.set_xlabel('t')
        ax_C.set_ylabel('M(t)')
        ax_C.plot(t.numpy(), pred_y.numpy()[:, 0, 0, 0], 'b--')

        fig.tight_layout()
        plt.savefig('png/{:03d}'.format(itr))

        np.savetxt('txt/SMCRD_NN' + args.areas + str(itr) + '.csv', pred_y.numpy()[:, 0, 0, :], delimiter=',')
