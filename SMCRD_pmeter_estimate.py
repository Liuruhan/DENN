...
if args.areas == 'wuhan':
    ...
    true_y0 = torch.tensor([[0.00027, 0.00003, 0.00, 0.00]]) # M, C, R, D
    N = 430.0003
    data = np.loadtxt('Piedmont_IRD.csv', delimiter=',', skiprows=1)
elif args.areas == 'AU_NSW':
    true_y0 = torch.tensor([[0.02394, 0.00266, 0.00, 0.00]]) # M, C, R, D
    N = 749.5586
    data = np.loadtxt('AU_NSW_IRD.csv', delimiter=',', skiprows=1)
elif args.areas == 'AU_VIC':
    true_y0 = torch.tensor([[0.01089, 0.00121, 0.00, 0.00]]) # M, C, R, D
    N = 593.2541
...
#countries
    ...
    true_y0 = torch.tensor([[0.00018, 0.00002, 0.00, 0.00]]) # I, R, D
    N = 5652.0042
...
print("Test:", all_t.size(), all_truey.size())
...
def visualize(true_y, pred_y, param, odefunc, itr, t):
    ...
    if args.viz:
        ...
        ax_M.set_xlabel('t')
        ax_M.set_ylabel('M(t)')
        ax_M.plot(t.numpy(), pred_y.numpy()[:, 0, 0, 0], 'b--')

        ax_C.cla()
        ax_C.set_title('C')
        ax_C.set_xlabel('t')
        ax_C.set_ylabel('C(t)')
        ax_C.plot(t.numpy(), pred_y.numpy()[:, 0, 0, 1], 'b--')
        ...
        np.savetxt('txt/SMCRD_pmter' + args.areas + str(itr) + '.csv', pred_y.numpy()[:, 0, 0, :], delimiter=',')
...
class ODEFunc(nn.Module):
    ...
    def forward(self, t, y):
        ...
        Y = torch.zeros([1,1,4])
        Y[:,:,0] = torch.abs(self.beta)*(N - y[:,:,0] - y[:,:,1] - y[:,:,2]-y[:,:,3]) * (y[:, :, 0]+y[:,:,1])/N - torch.abs(self.alpha)*y[:,:,0] - torch.abs(self.cita1)*y[:,:,0]
...
if __name__ == '__main__':
    ...
    func = ODEFunc()
    optimizer = optim.RMSprop([func.beta, func.alpha, func.cita1, func.cita2, func.ep], lr=args.lr)
    end = time.time()
    ...
    for itr in range(1, args.niters + 1):
        ...
        if itr % args.test_freq == 0:
            ...
            with torch.no_grad():
                ...
                loss = loss_I + loss_R + loss_D
                param = [func.beta.item(), func.alpha.item(), func.cita1.item(), func.cita2.item(), func.ep.item()]
